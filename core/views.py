...
def estado_resultados(request):
    """
    Estado de Resultados con cálculo de Utilidad Bruta, Operativa y Neta.
    """
    def saldo_cuenta(cuenta):
        t_debe = cuenta.movimientos.filter(tipo='debe').aggregate(
            total=Sum('monto'))['total'] or Decimal('0')
...
def chatbot_respuesta_local(user_message, cuentas_db, ctx):
    ...
    cuentas_por_codigo = {str(c['codigo']).strip(): c for c in cuentas_db}

    if any(palabra in texto for palabra in ('funcionas', 'puedo preguntar', 'estas activo', 'estas ahi', 'hola')):
        return (
            'Si, estoy activo. En este momento estoy en modo local porque no puedo conectar con Gemini desde este '
            'servidor, pero puedo revisar las cuentas registradas y ayudarte a validar asientos basicos.'
        )

    codigos = re.findall(r'\b\d{2,6}\b', user_message)
    if codigos:
        lineas = []
        for codigo in codigos:
            ...
            if cuenta:
                lineas.append(
                    f'La cuenta {codigo} existe: {cuenta["nombre"]} ({cuenta["tipo"]}).'
                )
            else:
                lineas.append(
                    f'No encuentro la cuenta {codigo} en tu catalogo local. Verifica si debes crearla antes de usarla en un asiento.'
                )
        lineas.append('Recuerda validar que el total Debe sea igual al total Haber antes de registrar el asiento.')
        return ' '.join(lineas)

    utilidad_neta = ctx.get('er_utilidad_neta', 0)
    return (
        f'Puedo ayudarte con consultas contables del sistema. Utilidad neta actual: S/ {utilidad_neta:,.2f}. '
        'Para validar un asiento, escribeme las cuentas, si van al Debe o Haber, y los montos.'
    )
...
def chatbot_api(request):
    ...
    Utiliza el System Prompt del usuario para validar PCGE, partida doble y presupuesto.
    """
    if request.method != 'POST':
    ...
    # 1. Obtener contexto del sistema para la IA
    try:
        ctx = get_reporte_context()
        # Obtener catálogo de cuentas para validación del PCGE
    ...
    # 2. SYSTEM PROMPT DEFINIDO POR EL USUARIO
    system_prompt = f"""
    ...
    CONTEXTO DEL SISTEMA LOCAL:
    - Utilidad Neta: S/ {ctx.get('er_utilidad_neta', 0):,.2f}
    - Cuentas ya creadas en este sistema:
    {resumen_cuentas}
    
    TU ROL: Experto en Contabilidad Computarizada y Auditoría.
    Aunque el usuario use cuentas básicas (10, 59), tú debes conocer y aplicar TODO el PCGE moderno.
    
    PROTOCOLOS:
    1. PCGE 2020: Si el usuario menciona una cuenta que no está en la lista de arriba, usa tu conocimiento experto del PCGE 2020 para responder.
    2. Partida Doble: Valida siempre que Debe = Haber.
    3. Registro Asistido: Genera asientos estructurados (ej. Compra: 60/4011/421).
    4. Advertencia Presupuestal: Si un monto supera S/ 1000, advierte.
    
    Responde de forma técnica, amable y precisa en español.
    """
    ...
    try:
        ...
        if response.status_code == 404:
            return JsonResponse({
                'error': f'El modelo "{model_name}" no esta disponible para esta API key. Prueba configurar GEMINI_MODEL=gemini-2.0-flash o GEMINI_MODEL=gemini-flash-latest.',
                'status': 'error'
            }, status=502)
